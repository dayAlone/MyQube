...
while ($cells = $res->Fetch())
{
    ...
    $row = array(
        ...
        $cells["a_name"],
        $cells["a_last_name"],
        $arEvent["PLACE_EVENT"]["VALUE"],
        $cells["UF_DATE_TIME"],
        $cells["UF_EVENT"],
        $cells["UF_USER"],
        $cells["u_name"],
        $cells["u_last_name"],
        ($cells["UF_TYPE_2"]==39) ? "" : $cells["u_email"],
        $cells["u_phone_1"]." ".$cells["u_phone_2"],
        implode(" ",unserialize($cells["UF_FB_PROFILE"])),
        implode(" ",unserialize($cells["UF_VK_PROFILE"])),
        implode(" ",unserialize($cells["UF_GP_PROFILE"])),
        $cells["u_birth_date"],
        $cells["UF_EVENT"] ? (($cells["UF_TYPE"]==34||$cells["UF_TYPE_2"]==39)?"1":"") : ($cells["UF_TYPE_2"]==39)?"1":"",
        $cells["UF_EVENT"] ? (($cells["UF_TYPE"]==36||$cells["UF_TYPE_2"]==41)?"1":"") : ($cells["UF_TYPE_2"]==41)?"1":"",
        $cells["UF_EVENT"] ? (($cells["UF_TYPE"]==35||$cells["UF_TYPE_2"]==40)?"1":"") : ($cells["UF_TYPE_2"]==40)?"1":"",
        $cells["UF_EVENT"] ? (($cells["UF_TYPE"]==37||$cells["UF_TYPE_2"]==42)?"1":"") : ($cells["UF_TYPE_2"]==42)?"1":"",
        $types[$cells["UF_TYPE"]] == 6 || $types[$cells["UF_TYPE_2"]] == 6 ? '1' : '',
    );
	if(!isset($_GET["csv"]))
	{?>
