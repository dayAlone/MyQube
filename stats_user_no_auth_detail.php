...
$APPLICATION->SetTitle("Статистика приглашений пользователей отказавшихся от регистрации детально");
...
$usersObj = CIBlockElement::GetList(array(), $arFilter, false,false, array("ID","ACTIVE","DATE_CREATE","PROPERTY_*"));
$counter = array();

while($user = $usersObj->Fetch()){	
	if($user["PROPERTY_123"]){		
		$counter[$user["PROPERTY_123"]][] = $user;
	}
}
...
<h2>
	<a href="stats_user_no_auth.php">Статистика приглашений пользователей отказавшихся от регистрации</a>
</h2>
<h2>
...
<thead><tr><th>Группа</th><th>ID</th><th>Логин</th><th>Имя пригласившего</th>
<th>ID</th><th>Дата регистрации</th><th>Активирован</th><th>Почта</th><th>Имя</th>
</tr>
...
foreach($table as $cells): ?>
	...
	<tr class="<?=($even ? "even" : "odd")?>">
		...
		<td class="count"><?=$valueList["DATE_CREATE"];?></td>
		<td class="count"><?=$valueList["ACTIVE"] == "Y" ? "Да" : "Нет";?></td>
		<td class="count">
			<?=$valueList["PROPERTY_116"];?>
		</td>
		<td class="count"><?=$valueList["PROPERTY_114"]."&nbsp;".$valueList["PROPERTY_115"];?></td>
	</tr>
		<?$even = !$even;?>
	<?endforeach;?>
<?endforeach;?>
