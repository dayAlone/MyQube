<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");?><?$_SESSION["BackFromDetail"]["group_6"]["nPageSize"] = $_GET["page"];

	$res = CIBlockElement::GetList(array("ACTIVE_FROM"=>"DESC"), array("IBLOCK_ID" => 1, "PROPERTY_SOCIAL_GROUP_ID" => $_GET["GROUP_ID"]),false, array("nPageSize" => 1, "nElementID" => $_GET["POST_ID"]), array("ID", "*"));
	...
	if((count($arPost) == 2) && ($_GET["POST_ID"] == $arPost[0]["ID"])) {	
		$this_post = 0;
		$close_url = '<a href="/mobile/'.$_GET["GROUP_ID"].'/#lenta_item_'.$arPost[$this_post]["ID"].'"><span class="icon_close"></span><b>Закрыть статью</b><div class="clear"></div></a>';
		$prev_url = '';
	...
	if($USER->IsAuthorized() /*&& CustomUser::UserYouHave18() && CustomUser::UserCheckFields()*/)
	{
		...
		<div class="detail_page" style="background-image: url('<?=CFile::GetPath($arPost[$this_post]["DETAIL_PICTURE"])?>');">
			<div class="detail_page_content">
				<h1><span class="color_blue"><?=$arPost[$this_post]["NAME"]?></span><br><?=$arPost[$this_post]["PROPERTIES"]["NAME_2"]["VALUE"]?></h1>
