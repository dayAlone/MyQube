<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");$APPLICATION->SetTitle("Новый супервайзер");?>
<?if(in_array(10,$Data["User"]["Group"]) || in_array(1,$Data["User"]["Group"])):?>
	<?
		...
		if(isset($_POST["Form"])){
			...
			if($Form["UserId"] != -1){
				$Query = CUser::GetList(($by="id"), ($order="desc"), array("ID"=>$Form["UserId"],"GROUPS_ID"=>12))->Fetch();
				if(!empty($Query)){
					$Form["Fields"]["Email"] = $Query["EMAIL"];
					$Form["Fields"]["Login"] = $Query["LOGIN"];
					$Form["Fields"]["City"] = $Query["WORK_CITY"];
					$Form["Fields"]["Mobile"] = $Query["PERSONAL_MOBILE"];
				} else {
					$Form["Action"] = true;
					$Form["UserId"] = -1;
				}
		...
		if($Form["IsFinal"]){
			LocalRedirect("/dashboard/list_supervisor/");
		}
	?>
	...
	<form method="POST">
		...
		<table class="table_new_cons" cellpadding="0" cellspacing="0">
			...
			<tr>
				<td>E-Mail</td>
				<td>
					<input type="text" disabled="disabled" name="Form[Email]" value="<?=$Form["Fields"]["Email"];?>"/>
					<input type="hidden" name="Form[Email]" value="<?=$Form["Fields"]["Email"];?>"/>
			...
			<tr>
				<td>Мобильный</td>
				<td>
					<input type="text" name="Form[Mobile]" value="<?=$Form["Fields"]["Mobile"];?>"/>
				</td>
			</tr>
			<tr>
				<td colspan="2">
