	...
	<div class="socwrap">
		...
		<?
			if (is_array($arResult["BOOKMARKS"]) && count($arResult["BOOKMARKS"]) > 0)
			{
				foreach($arResult["BOOKMARKS"] as $name => $arBookmark)
				{
					if(strripos($arBookmark["ICON"], 'class="facebook"'))
						echo str_replace('class="facebook"', 'class="facebook share_click"', $arBookmark["ICON"]);
					elseif(strripos($arBookmark["ICON"], 'class="vk"'))					
						echo str_replace('class="vk"', 'class="vk share_click"', $arBookmark["ICON"]);
					elseif(strripos($arBookmark["ICON"], 'class="google"'))					
						echo str_replace('class="google"', 'class="google share_click"', $arBookmark["ICON"]);
				}
			}
		?>
