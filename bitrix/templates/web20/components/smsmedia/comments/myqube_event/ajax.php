		...
		if(!empty($_POST['TEXT']))
		{
			...
			if(strlen($_POST['TEXT']) && !empty($user) && empty($_POST['testForm']))
			{
				...
				if($NEW_ID = $el->Add($arLoadProductArray))
				{
					...
					echo '<div class="comment flex-between" id="comment_'.$NEW_ID.'" style="'.$otstup.'">'.$reaply;
?>
...
<?
					...
					$username = ($USER->GetFirstName()!="")?($USER->GetFirstName()." ".$USER->GetLastName()):$USER->GetLogin();
					$username_answer = $username;
					?>
						<div class="gallery_comm_item_info">
							...
							<div class="gallery_comm_item_text"><?=$_POST['TEXT']?></div>
							<div class="otvet" id="reply_to_<?=$NEW_ID?>">
								<a href="javascript:ReplyToComment('<?=$NEW_ID?>', '<?=$username?>, ', '<?=$_POST['depth_level']?>');" class="answer_link<?=$NEW_ID?>"><?=GetMessage('REPLY')?>Ответить</a>
							</div>
						</div>
						<div class="clear"></div>
					...
					if(isset($_POST['PARENT_ID_W']))
					{
						$infoblock_id_tmp = 7;
						$post_id_tmp=intval($_POST['PARENT_ID_W']);
